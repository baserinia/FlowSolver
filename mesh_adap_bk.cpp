...
NIFS::c_mesh_adap::delaunay( c_face* face )
{
	...
	bnd_vert_temp[0] = static_cast<c_vertex*>( face->get_vert( 0 ) );
	metric[0] = get_size_matrix( bnd_vert_temp[0]->get_point().get_pos() );
	bnd_vert_temp[1] = static_cast<c_vertex*>( face->get_vert( 1 ) );
	metric[1] = get_size_matrix( bnd_vert_temp[1]->get_point().get_pos() );
	for ( int i = 0 ; i < 3 ; ++i) {
		if ( ( cell_temp[0]->get_vert(i) != bnd_vert_temp[0] ) &&
			 ( cell_temp[0]->get_vert(i) != bnd_vert_temp[1] ) ) {
			bnd_vert_temp[2] = static_cast<c_vertex*>( cell_temp[0]->get_vert(i) );
			metric[2] = get_size_matrix( bnd_vert_temp[2]->get_point().get_pos() );
		}
		if ( ( cell_temp[1]->get_vert(i) != bnd_vert_temp[0] ) &&
			 ( cell_temp[1]->get_vert(i) != bnd_vert_temp[1] ) ) {
			bnd_vert_temp[3] = static_cast<c_vertex*>( cell_temp[1]->get_vert(i) );
			metric[3] = get_size_matrix( bnd_vert_temp[3]->get_point().get_pos() );
		}
	}
...
NIFS::c_mesh_adap::smooth_vertex( c_vertex* vert )
{
  ...
  eig = get_size_matrix( vert->get_point().get_pos() ).eig_val();
  eig_1 = std::abs( eig(0)(0) );
  eig_2 = std::abs( eig(1)(1) );
  avg = ( eig_1 < eig_2 ) ? ( eig_2/eig_1) : ( eig_1/eig_2);
  ratio = ( eig_1 < eig_2 ) ? ( 1.0/eig_2) : ( 1.0/eig_1);
...
//      cond = 0;
    ...
    if ( vert->get_type() != 0 ) { // vert is boundary
      if ( vert_ptr->get_type() == 2 )
        cond = 2; // if vert is boundary but face is internal
/*
      if ( vert_ptr->get_type() == 0 )
        cond = 0; // if vert is boundary but face is internal
      for ( unsigned j = 0 ; j < vert->get_face_num() ; ++j ) {
        face_ptr = static_cast<c_face*>( vert->get_face( j ) );
        if ( ( face_ptr->get_type() == 0 ) &&
             ( ( face_ptr->get_vert( 0 ) == vert_ptr ) ||
               ( face_ptr->get_vert( 1 ) == vert_ptr ) ) )
          cond = 0;
      }
*/      
		}
...
//		r = m_bg_mesh->get_length( vert->get_point().get_pos() ,
//		                           vert_ptr->get_point().get_pos() );
//		avg += r;
//    d = ( vert_ptr->get_point().get_pos() - vert->get_point().get_pos() ).get_magnit() / r;
...
//    f = - ( 1.0 - std::pow( d , 10.0 ) ) / ( 1.0 + std::pow( d , 20.0 ) );
    }
...
//  avg /= static_cast<double>( n );
  del_vec = vec * avg / static_cast<double>( n );// + vert->get_point().get_pos();
...
NIFS::c_mesh_adap::modify()
{
  ...
  n = 0;
  for ( unsigned j = 0; j < get_vert_num() ; ++j ) {
    vert_ptr = verts[j];
    cond = false;
...
//      r =  ( vert_ptr_0->get_point().get_pos() - 
//                  vert_ptr_1->get_point().get_pos() ).get_magnit() /
//            m_bg_mesh->get_length( vert_ptr_0->get_point().get_pos(),
...
NIFS::c_mesh_adap::switch_quad2()
{
    ...
    if ( fabs( vec_0.get_unit() * vec_1.get_unit() ) > 0.5 )
      continue;
//    std::cout << n << std::endl;
//    if ( merge_tri_to_quad( face_ptr ) > 0 );
//      break;
    for ( unsigned i = 0 ; i < 3 ; ++i ) {
      face_ptr_temp = static_cast<c_face*>( cell_ptr_0->get_face( i ) );
      if ( ( face_ptr != face_ptr_temp ) &&
           ( face_ptr->get_area() < face_ptr_temp->get_area() ) )
        cond = false;
    }
    for ( unsigned i = 0 ; i < 3 ; ++i ) {
      face_ptr_temp = static_cast<c_face*>( cell_ptr_1->get_face( i ) );
      if ( ( face_ptr != face_ptr_temp ) &&
           ( face_ptr->get_area() < face_ptr_temp->get_area() ) )
        cond = false;
    }
...
//	for ( unsigned i = 0 ; i < cell_ptr_0->get_cell_num() ; ++i )
//	  switch_quad_nbr( static_cast<c_cell*>( cell_ptr_0->get_cell(i) ) );
//	
	return;
}
...
//      break;
    ...
    r=0;
    for ( unsigned i = 0 ; i < cell_ptr_0->get_cell_num() ; ++i ) {
      cell_ptr = static_cast<c_cell*>( cell_ptr_0->get_cell( i ) );
      if ( cell_ptr->get_face_num() == 4 ) {
        cells[p] = cell_ptr;
        ++p;
      }
    }
    for ( unsigned i = 0 ; i < cell_ptr_1->get_cell_num() ; ++i ) {
      cell_ptr = static_cast<c_cell*>( cell_ptr_1->get_cell( i ) );
      if ( cell_ptr->get_face_num() == 4 ) {
        cells[q+2] = cell_ptr;
        ++q;
    ...
    if ( p > 0 ){
      for ( unsigned i = 0 ; i < 3 ; ++i ) {
        face_ptr_temp = static_cast<c_face*>( cell_ptr_0->get_face( i ) );
        if ( ( face_ptr != face_ptr_temp ) &&
             ( cells[0]->is_face_nbr( face_ptr_temp ) == false ) &&
             ( face_ptr->get_area() < face_ptr_temp->get_area() ) )
          cond1 = false;
      }
    } else {
    ...
    if ( q > 0 ){
      for ( unsigned i = 0 ; i < 3 ; ++i ) {
        face_ptr_temp = static_cast<c_face*>( cell_ptr_1->get_face( i ) );
        if ( ( face_ptr != face_ptr_temp ) &&
             ( cells[2]->is_face_nbr( face_ptr_temp ) == false ) &&
             ( face_ptr->get_area() < face_ptr_temp->get_area() ) )
          cond2 = false;
      }
    } else {
...
int
NIFS::c_mesh_adap::switch_quad_nbr( c_cell* cell )
{
  if ( cell->get_face_num() != 3 )
    return -1;
  if ( cell->get_aspect_ratio() < 1.5 )
    return -1;
    
  c_cell* cell_ptr;
  c_face* face_ptr;
  std::vector< c_cell* > cell_temp( 3 );  
  c_face* face_rem = NULL;
  int n = 0;
  int i;
  double len = 0;
  
  for ( i = 0 ; i < cell->get_cell_num() ; ++i )
  {
    cell_ptr = static_cast<c_cell*>( cell->get_cell( i ) );
    if ( cell_ptr->get_face_num() == 4 ) {
      cell_temp[n] = cell_ptr;
      ++n;
    }
  }
  ...
  if ( n == 0 )
    ...
    for ( i = 0 ; i < cell->get_face_num() ; ++i )
    {
      face_ptr = static_cast<c_face*>( cell->get_face( i ) );
      if ( ( cell_temp[0]->is_face_nbr( face_ptr ) == false ) &&
           ( face_ptr->get_area() > len ) )
      {
        len = face_ptr->get_area();
        face_rem = face_ptr;
      }
    }
    cell_ptr = static_cast<c_cell*>( face_rem->get_cell(0) );
    if ( merge_tri_to_quad( face_rem) > 0  )
      for ( i = 0 ; i < cell_ptr->get_cell_num() ; ++i )
        switch_quad_nbr( static_cast<c_cell*>( cell_ptr->get_cell( i ) ) );
  }
  else if ( n == 2 ) {
    for ( i = 0 ; i < cell->get_face_num() ; ++i )
    {
      face_ptr = static_cast<c_face*>( cell->get_face( i ) );
      if ( ( cell_temp[0]->is_face_nbr( face_ptr ) == false ) &&
           ( cell_temp[1]->is_face_nbr( face_ptr ) == false ) )
      {
        face_rem = face_ptr;
      }
    }
...
NIFS::c_mesh_adap::cleanup_tri( void )
{
    ...
    cond = false;
/*    std::cout << n<<' '<< face_ptr << ' ' <<
    static_cast<c_vertex*>(face_ptr->get_vert(0))->get_point().get_pos().get_x() << ' '<<
    static_cast<c_vertex*>(face_ptr->get_vert(0))->get_point().get_pos().get_y() << ' '<<
    static_cast<c_vertex*>(face_ptr->get_vert(1))->get_point().get_pos().get_x() << ' '<<
    static_cast<c_vertex*>(face_ptr->get_vert(1))->get_point().get_pos().get_y() 
    << std::endl;      */
    if ( face_ptr->get_cell_num() == 2 ) {
      cell_ptr_0 = static_cast<c_cell*>( face_ptr->get_cell( 0 ) );
      cell_ptr_1 = static_cast<c_cell*>( face_ptr->get_cell( 1 ) );
